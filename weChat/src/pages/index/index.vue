<template>
 <div class="wrap">
    <scroll-view class="headers" scroll-x>
      ...
      <ul class="ulList">
        <li v-for="(item,index) in typeList" :key="index" @click="actives(index)" :index="index">{{item.cname}}</li>
      </ul>
    </scroll-view>
    <Login/>
    <div>
      <Daily
        :swriperData="swriperData"
        :threeArr="threeArr"
        :spikeArr="spikeArr"
        :arrr3="arrr3"
        :arrr1="arrr1"
    ...
    wx.request({
      ...
      success(res) {
        ...
        res.data.result.map((item, index) => {
          ...
          } else if (item.type == "carouselProduct") {
            //一拖三的一
            //一拖三的三
            that.arrr1.push(item);
            that.arrr3.push(item.items);
